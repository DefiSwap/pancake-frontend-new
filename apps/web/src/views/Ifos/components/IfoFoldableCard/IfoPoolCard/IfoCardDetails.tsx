...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ...
      : t(
          'For the basic sale, Max CAKE entry is capped by minimum between your average PATTIE Balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake more CAKE into the iCAKE',
        )
  ...
  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      ...
      t(
        'For the unlimited sale, Max CAKE entry is capped by your average PATTIE Balance in the iCake. To increase the max entry, Stake more CAKE into the iCake',
      )
    )
